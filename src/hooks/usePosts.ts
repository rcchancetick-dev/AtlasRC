...
export function usePosts(admin = false) {
  ...
  const reload = useCallback(async () => { if (!configured) { setError('Configurez Supabase pour afficher les contenus.'); setLoading(false); return; } const {data,error} = await supabase.from('posts').select('*').order('created_at',{ascending:false}).limit(500); if(error) setError(error.message); else {setPosts((data||[]) as Post[]);setError('')} setLoading(false); },[]);
  useEffect(()=>{void reload(); const channel = supabase.channel('atlas-posts').on('postgres_changes',{event:'*',schema:'public',table:'posts'},payload=>{if(payload.eventType==='INSERT') setFresh((payload.new as Post).title);void reload()}).subscribe(); return ()=>{void supabase.removeChannel(channel)}},[reload]);
